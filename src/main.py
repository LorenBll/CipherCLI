...
def _load_configuration() -> dict:
	...
	try:
		...
		log_info("Configuration loaded", {"path": str(CONFIG_PATH)})
	except json.JSONDecodeError as exc:
		log_error("Configuration file contains invalid JSON", {"error": str(exc)})
		raise CipherCliError("Configuration file contains invalid JSON.") from exc
	except OSError as exc:
		log_error("Failed to read configuration file", {"error": str(exc)})
		raise CipherCliError("Failed to read configuration file.") from exc

	if not isinstance(config, dict):
		log_warn("Configuration root is not a JSON object")
...
def _parse_config_port(value: object, field_name: str, default_value: int) -> int:
	...
	if not isinstance(value, int):
		log_warn("Configuration field must be an integer", {"field": field_name, "value": value})
		raise CipherCliError(f"{field_name} in configuration.json must be an integer.")

	if value < 1 or value > 65535:
		log_warn("Configuration field out of range", {"field": field_name, "value": value})
...
def _parse_config_bool(value: object, field_name: str, default_value: bool) -> bool:
	...
	log_warn(
		...
		{"field": field_name, "type": type(value).__name__, "default": default_value},
	)
	return default_value
...
def _test_port_open(host: str, port: int, timeout: float = 2.0) -> bool:
	...
	try:
		with socket.create_connection((host, port), timeout=timeout):
			log_debug("Port test open", {"host": host, "port": port})
			return True
	except (OSError, socket.timeout):
		log_debug("Port test closed", {"host": host, "port": port})
		return False
...
def _resolve_service_port(service_name: str, config_port: int, servicehandler_port: int, servicehandler_enabled: bool) -> int:
	...
	if _test_port_open(LOOPBACK_HOST, config_port):
		log_info("Using configured port", {"service": service_name, "port": config_port})
		return config_port

	log_warn("Configured port unreachable", {"service": service_name, "port": config_port})

	if servicehandler_enabled:
		log_info("Querying ServiceHandler for port", {"service": service_name})
		try:
			...
			if response.status_code == 200 and isinstance(response.json_body, dict):
				...
				if isinstance(port, int) and 1 <= port <= 65535:
					log_info("Resolved port via ServiceHandler", {"service": service_name, "port": port})
					return port
		except CipherCliError as exc:
			log_warn("ServiceHandler resolution failed", {"service": service_name, "error": str(exc)})

	log_debug("Falling back to configured port", {"service": service_name, "port": config_port})
	return config_port
...
def _send_post_json(http_request: PostRequest) -> PostResponse:
	...
	log_debug("Sending POST request", {"url": http_request.url})
	request_headers = {
	...
	try:
		with request.urlopen(urllib_request, timeout=http_request.timeout) as response:
			...
			log_debug("POST response received", {"url": http_request.url, "status": response.status})
			return PostResponse(
		...
		log_debug("POST HTTP error received", {"url": http_request.url, "status": exc.code})
		return PostResponse(
		...
		log_warn("POST failed to contact service", {"url": http_request.url, "error": str(exc.reason)})
...
def _send_get_json(http_request: GetRequest, body: dict | None = None) -> GetResponse:
	...
	log_debug("Sending GET request", {"url": http_request.url})
	request_headers = {
		"Content-Type": "application/json",
		"Accept": "application/json",
		"Connection": "close",
	...
	try:
		with request.urlopen(urllib_request, timeout=http_request.timeout) as response:
			...
			response_body = response_body_bytes.decode("utf-8", errors="replace")
			parsed_json = _try_parse_json(response_body)
			log_debug("GET response received", {"url": http_request.url, "status": response.status})
			return GetResponse(
				status_code=response.status,
				body=response_body,
				json_body=parsed_json,
		...
		error_body_bytes = exc.read()
		error_body = error_body_bytes.decode("utf-8", errors="replace")
		log_debug("GET HTTP error received", {"url": http_request.url, "status": exc.code})
		return GetResponse(
			status_code=exc.code,
			body=error_body,
			json_body=_try_parse_json(error_body),
		...
		log_warn("GET failed to contact service", {"url": http_request.url, "error": str(exc.reason)})
		raise CipherCliError(f"Failed to contact service at {http_request.url}: {exc.reason}") from exc
...
def _try_parse_json(payload_text: str) -> dict | list | str | int | float | bool | None:
	...
	try:
		...
		log_debug("Failed to parse JSON from payload")
		return None
...
def _resolve_ultimate_path_to_raw(path_text: str, diskidentifier_port: int) -> Path:
	...
	log_debug("Resolving ultimate path", {"path": path_text})
	...
	log_debug("Resolved ultimate path", {"path": str(resolved)})
	return resolved
...
def _normalize_cli_path(path_text: str, diskidentifier_port: int) -> Path:
	...
	if _looks_like_ultimate_path(candidate):
		...
		log_debug("Normalized ultimate path", {"input": path_text, "resolved": str(resolved)})
		return resolved
	...
	log_debug("Normalized path", {"input": path_text, "resolved": str(resolved)})
	return resolved
...
def _validate_ck_arguments(path_argument: str, optional_file_name: str | None) -> tuple[Path, str]:
	...
	if optional_file_name is None:
		...
		if not destination_path.parent.exists() or not destination_path.parent.is_dir():
			log_warn("ck destination directory does not exist")
			raise CipherCliError("The destination directory does not exist.")
		file_name = destination_path.name
		if not file_name:
			log_warn("ck path has an empty file name")
			raise CipherCliError("The provided path must include a file name.")
	...
	if not base_directory.exists() or not base_directory.is_dir():
		log_warn("ck base directory does not exist")
	...
	if safe_name != optional_file_name or optional_file_name in {".", ".."}:
		log_warn("ck file name contains path components")
	...
	if destination_path.exists() and destination_path.is_dir():
		log_warn("ck target points to a directory")
		raise CipherCliError("The target points to a directory.")
	if destination_path.exists():
		log_warn("ck target file already exists")
...
def _run_ck_mode(args: argparse.Namespace, cipher_port: int, diskidentifier_port: int) -> int:
	...
	log_info("Starting key-creation mode", {"path": normalized_path_str})
	...
	log_info("Creating key", {"path": str(directory_path / file_name)})
	...
	log_info("Key creation request returned", {"status": response.status_code})

	if response.status_code == 201:
		log_info("Key created successfully", {"path": str(directory_path / file_name)})
...
def _report_error(message: str, level: str = "WARN") -> None:
...
def _parse_files_list(file_path: str) -> list[str]:
	...
	log_debug("Loaded paths from files list", {"count": len(paths)})
	return paths
...
def _normalize_existing_file_path(path_text: str, diskidentifier_port: int, field_name: str) -> Path:
	...
	log_debug("Normalized file path", {"field": field_name, "path": str(normalized)})
	return normalized
...
def _poll_task_until_done(task_id: str, cipher_port: int, operation: str) -> int:
	...
	while True:
		attempt += 1
		log_debug("Polling task", {"task_id": task_id, "attempt": attempt})
		...
		if not isinstance(response.json_body, dict):
			_report_error("Task endpoint returned an invalid payload.", level="ERROR")
			return 1

		status = response.json_body.get("status")
		if not isinstance(status, str) or not status.strip():
			_report_error("Task status is missing in server response.", level="ERROR")
			return 1

		status = status.strip()
		if status != last_status:
			log_info("Task status update", {"task_id": task_id, "status": status})
			print(f"Task {task_id}: {status}")
			last_status = status

		if status == "completed":
			log_debug("Task reached terminal state", {"task_id": task_id, "status": status})
				...
				if isinstance(files, list) and files:
					for file_entry in files:
						...
						if isinstance(input_path, str) and isinstance(output_path, str):
							log_debug("Task file mapping", {"input_path": input_path, "output_path": output_path})
							print(f"{input_path} -> {output_path}")
			log_info("Task completed", {"operation": operation, "task_id": task_id})
		...
		if status == "failed":
			...
			if isinstance(error_message, str) and error_message.strip():
				...
				_report_error(f"{operation} task failed.", level="ERROR")
			return 1
...
def _run_cipher_mode(
	...
	log_info("Starting mode", {"operation": operation, "endpoint": endpoint})
	...
	if file_name_flag_value:
		if output_file_path_arg:
			log_warn("Flag conflict", {"flags": [f"--{file_name_flag_dash}", "--output-file-path"]})
			raise CipherCliError(f"--{file_name_flag_dash} cannot be combined with --output-file-path.")
		if output_file_paths_arg:
			log_warn("Flag conflict", {"flags": [f"--{file_name_flag_dash}", "--output-file-paths"]})
	...
	if output_dir_arg:
		if overwrite:
			log_warn("Flag conflict", {"flags": ["--output-dir", "--overwrite-file"]})
			raise CipherCliError("--output-dir cannot be combined with --overwrite-file.")
		if output_file_path_arg:
			log_warn("Flag conflict", {"flags": ["--output-dir", "--output-file-path"]})
			raise CipherCliError("--output-dir cannot be combined with --output-file-path.")
		if output_file_paths_arg:
			log_warn("Flag conflict", {"flags": ["--output-dir", "--output-file-paths"]})
	...
	if not file_name_flag_value and not overwrite:
		if not output_file_path_arg and not output_file_paths_arg and not output_dir_arg:
			log_warn("No output path specified", {"operation": operation})
	...
	if output_dir_arg:
		...
		if len(output_file_paths_arg) != len(file_paths):
			log_warn("Output file paths count mismatch", {"output_count": len(output_file_paths_arg), "input_count": len(file_paths)})
		...
		if len(file_paths) != 1:
			log_warn("Output file path used with multiple input files", {"input_count": len(file_paths)})
	...
	log_info("Queuing task", {"operation": operation, "file_count": len(file_paths)})
	...
	response = _send_post_json(post_request)
	log_info("Request returned", {"operation": operation, "status": response.status_code})
	...
	if not isinstance(response.json_body, dict):
		_report_error("Cipher service returned an invalid task payload.", level="ERROR")
		return 1

	task_id = response.json_body.get("task_id")
	if not isinstance(task_id, str) or not task_id.strip():
		_report_error("Cipher service did not return a task id.", level="ERROR")
		return 1

	task_id = task_id.strip()
	log_info("Task queued", {"task_id": task_id, "operation": operation})
...
def main() -> int:
	...
	try:
		...
		log_info("Resolved ports", {"cipher": cipher_port, "diskidentifier": diskidentifier_port})
	except CipherCliError as exc:
	...
	if len(sys.argv) == 1:
		log_debug("No arguments provided, printing help")
	...
	if args.mode == "ck":
		log_info("Dispatching to ck mode")
		try:
			ck_result = _run_ck_mode(args, cipher_port, diskidentifier_port)
			log_info("Exiting", {"code": ck_result})
			return ck_result
		except CipherCliError as exc:
			_report_error(str(exc))
			log_info("Exiting", {"code": 1})
	...
	if args.mode == "c":
		log_info("Dispatching to c mode")
		try:
			if args.key_path is None:
				log_warn("Encrypt mode validation failed", {"reason": "key_path is missing"})
				raise CipherCliError("Both key_path and at least one file_path are required.")
			if args.files_list:
				if args.file_paths:
					log_warn("Encrypt mode validation failed", {"reason": "--files-list combined with positional file paths"})
				...
				log_warn("Encrypt mode validation failed", {"reason": "no file paths provided"})
				raise CipherCliError("Both key_path and at least one file_path are required.")
			...
			log_info("Exiting", {"code": cipher_result})
	...
	if args.mode == "d":
		log_info("Dispatching to d mode")
		try:
			if args.key_path is None:
				log_warn("Decrypt mode validation failed", {"reason": "key_path is missing"})
				raise CipherCliError("Both key_path and at least one file_path are required.")
			if args.files_list:
				if args.file_paths:
					log_warn("Decrypt mode validation failed", {"reason": "--files-list combined with positional file paths"})
					raise CipherCliError("--files-list cannot be combined with positional file paths.")
				args.file_paths = _parse_files_list(args.files_list)
			elif not args.file_paths:
				log_warn("Decrypt mode validation failed", {"reason": "no file paths provided"})
				raise CipherCliError("Both key_path and at least one file_path are required.")
			cipher_result = _run_cipher_mode(
				args,
				cipher_port,
	...
	if args.mode == "health":
		log_info("Dispatching to health mode")
		try:
			health_result = _run_health_mode(cipher_port)
			log_info("Exiting", {"code": health_result})
	...
	log_debug("Unknown mode", {"mode": args.mode})
	parser.print_help()
	log_info("Exiting", {"code": 1})
	return 1
...
if __name__ == "__main__":
	...
	init_logging("CipherCLI", debug=early_args.debug)
	log_info("CipherCLI invoked", {"args": " ".join(sys.argv)})
